...
\section{Complete Trajectory Comparison Across Architectures and Wind Conditions}
\label{sec:cmg_improvement}

\begin{figure}[H]
    \centering
    \includegraphics[width=1.1\linewidth]{resources/img/appendix/trajectory_grid_pub.pdf}
    \caption{Trajectory comparison across different architectures and wind conditions.}
    \label{fig:trajectory_grid}
\end{figure}

The trajectory plots provide a qualitative comparison of the steering behaviour exhibited by the three control architectures under varying wind conditions. The Baseline controller produces the most linear and geometrically smooth trajectories; however, it demonstrates increased drift under high-wind scenarios, suggesting limited adaptability to rapidly changing environmental forces. The MLP-based controller exhibits noticeable instability, particularly in low-wind cases (e.g., H:85$^\circ$ | W:10\,kt), where large oscillations and inefficient path deviations indicate insufficient control damping and reduced trajectory efficiency. 

In contrast, the proposed 1DCNN architecture generates the most consistent and robust trajectories across the entire evaluation grid. Even under challenging conditions such as W:20\,kt, the 1DCNN maintains tighter path adherence with reduced lateral deviations compared to the MLP model. These results suggest an improved capability to capture and respond to the nonlinear aerodynamic and hydrodynamic dynamics inherent to high-performance foiling sailboats.
