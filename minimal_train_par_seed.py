import json
import random
import numpy as np
import torch
from pathlib import Path
from tqdm.auto import tqdm
from typing import Callable


# Gymnasium & SB3 Imports
from gymnasium.wrappers import FlattenObservation
from stable_baselines3 import PPO
from stable_baselines3.common.callbacks import BaseCallback, EvalCallback

from stable_baselines3.common.callbacks import CheckpointCallback 

from stable_baselines3.common.env_util import make_vec_env
from stable_baselines3.common.monitor import Monitor
from stable_baselines3.common.vec_env import VecNormalize
from stable_baselines3.common.utils import set_random_seed

# Custom Environment Imports
from boatsgym.envs.consigne.sailboat_consigne import SailboatEnv_consigne
from boatsimulator.core.gl.contextmanager import ContextManager

# --- IMPORT OUR MODULAR PARALLEL CLASS ---
from parallel_env import NonDaemonicSubprocVecEnv

# -------------------- 1. Linear Learning Rate Scheduler --------------------
'''
def linear_schedule(initial_value: float) -> Callable[[float], float]:
    """
    Linear learning rate schedule.
    :param initial_value: The initial learning rate.
    :return: schedule that computes current learning rate depending on remaining progress
    """
    def func(progress_remaining: float) -> float:
        # progress_remaining starts at 1.0 (beginning) and goes to 0.0 (end)
        return progress_remaining * initial_value
    return func
'''

def linear_schedule(initial_value: float, final_value: float = 0.0) -> Callable[[float], float]:
    """
    Linear learning rate schedule.
    
    :param initial_value: The initial learning rate.
    :param final_value: The final learning rate.
    :return: schedule that computes current learning rate depending on remaining progress
    """
    def func(progress_remaining: float) -> float:
        # progress_remaining starts at 1.0 (start) and linearly decays to 0.0 (end)
        # Formula: end + progress * (start - end)
        return final_value + progress_remaining * (initial_value - final_value)
    
    return func


# -------------------- Callbacks --------------------

class ProgressBarCallback(BaseCallback):
    """
    Updates a TQDM progress bar during training.
    """
    def __init__(self, pbar):
        super().__init__()
        self._pbar = pbar

    def _on_step(self) -> bool:
        self._pbar.n = self.num_timesteps
        self._pbar.update(0)
        return True

class SyncVecNormalizeCallback(BaseCallback):
    """
    Callback that synchronizes the normalization statistics (obs_rms)
    from the training environment to the evaluation environment.
    """
    def __init__(self, train_env, eval_env):
        super().__init__()
        self.train_env = train_env
        self.eval_env = eval_env

    def _on_step(self) -> bool:
        # Copy the Running Mean/Std of observations from Train to Eval
        # We generally do NOT sync reward stats (ret_rms) because 
        # evaluation rewards should be raw/unnormalized for human readability.
        self.eval_env.obs_rms = self.train_env.obs_rms
        return True

# -------------------- Seeding --------------------

def set_all_seeds(seed: int):
    """
    Sets seeds for all relevant random number generators to ensure reproducibility.
    """
    set_random_seed(seed)
    np.random.seed(seed)
    random.seed(seed)
    torch.manual_seed(seed)
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(seed)

# -------------------- Environment Factory --------------------

def make_env(params):
    """
    Factory function required by make_vec_env.
    Returns a function that initializes the environment.
    """
    def _init():
        cm = ContextManager(headless=True)
        env = SailboatEnv_consigne("Test env.", params, cm=cm)
        env = Monitor(env)
        env = FlattenObservation(env)
        return env
    return _init

# -------------------- Main Execution --------------------

if __name__ == '__main__':

    # ===== Experiment Configuration =====
    TRAIN_STEPS = 250000 #100000
    N_PROCS = 5  # Number of parallel processes
    #SEEDS = [0, 1, 2]  # List of seeds to run
    SEEDS = [0,1]
    
    # Base directory for all runs
    BASE_DIR = Path("model") / f"Par_{TRAIN_STEPS}_MLP"
    BASE_DIR.mkdir(parents=True, exist_ok=True)
    
    with open("train.json") as f:
        params = json.load(f)

    # ===== Run Loop for Multiple Seeds =====
    for seed in SEEDS:
        print(f"\n==============================")
        print(f" Running seed {seed}")
        print(f"==============================")

        # Create a specific directory for this seed
        run_dir = BASE_DIR / f"seed_{seed}"
        run_dir.mkdir(parents=True, exist_ok=True)

        # 1. Set global seeds
        set_all_seeds(seed)

        # 2. Create Training Environment
        print(f"Initializing training environments (N={N_PROCS})...")
        train_env = make_vec_env(
            make_env(params), 
            n_envs=N_PROCS, 
            seed=seed, 
            vec_env_cls=NonDaemonicSubprocVecEnv
        )
        
        # Wrap with VecNormalize (Train=True)
        # This learns the mean/std of observations and rewards
        train_env = VecNormalize(train_env, norm_obs=True, norm_reward=True, clip_obs=10.)

        # 3. Create Evaluation Environment
        # Use a distinct seed (e.g., +1000) so validation conditions are unique but reproducible
        # 2. Create Evaluation Environment ### IMPORTANT --> EVAL LOGIC IS REMOVED SINCE WE ARE USING CUSTOM CHEACK
        #POINTS        
        #print("Initializing evaluation environment...")
        #eval_env = make_vec_env(
        #    make_env(params), 
        #    n_envs=1, 
        #    seed=seed + 1000,
            # Standard DummyVecEnv is usually fine for eval, but we can use subproc if needed
        #)
        
        # Wrap with VecNormalize (Train=False)
        # This USES the stats from training but does not update them
        #comment eval env becuase we are not using it with checkpoint method
        #eval_env = VecNormalize(eval_env, norm_obs=True, norm_reward=False, training=False, clip_obs=10.)

        # 4. Configure Callbacks
        
        # A. Sync Stats: Keeps eval normalization up to date with training
        #sync_cb = SyncVecNormalizeCallback(train_env, eval_env)

        # B. Eval Callback: Periodically evaluates the agent
        checkpoint_cb = CheckpointCallback(
            save_freq= 1000,              # Save every 10,000 timesteps
            save_path=str(run_dir / "cheakpoints"),
            name_prefix="ppo_sailboat",        
            save_vecnormalize=True        # CRITICAL: Saves the environment stats (mean/std)
        )

        # Define a larger network architecture
        policy_kwargs = dict(
            net_arch=dict(pi=[64, 64], vf=[64, 64]) # 2 layers of 128 neurons
        )

        lr_schedule = linear_schedule(1e-3, 1e-5)


        # 5. Initialize Model (MlpPolicy as per original code)
        model = PPO(
            'MlpPolicy',
            train_env,
            learning_rate=lr_schedule,
            n_steps=128,
            batch_size=128,
            gamma=0.995,        # Higher gamma for longer episodes (look further ahead)
            gae_lambda=0.92, #0.95
            ent_coef=0.01,      # Encourage exploration
            policy_kwargs=policy_kwargs,
            verbose=1,
            tensorboard_log=str(run_dir)
        )

        # 6. Start Learning
        print(f"Start learning on {N_PROCS} cores for seed {seed}")
        with tqdm(total=TRAIN_STEPS, desc=f"Training Seed {seed}") as pbar:
            model.learn(
                total_timesteps=TRAIN_STEPS, 
                # Combine callbacks: Progress Bar -> Sync Norm Stats -> Run Evaluation
                callback=[ProgressBarCallback(pbar), checkpoint_cb], # No EvalCallback
                tb_log_name="tb_mlp",
                reset_num_timesteps=True
            )
        
        print(f"End of training for seed {seed}")

        # 7. Save Final Results
        SAVED_MODEL_NAME = run_dir / f"mlp_model_ns128.zip"
        STATS_PATH = run_dir / "vec_normalize_ns128.pkl"

        train_env.save(str(STATS_PATH))
        print(f"Normalization stats saved to {STATS_PATH}")

        model.save(SAVED_MODEL_NAME)
        print(f"Model saved successfully as {SAVED_MODEL_NAME}")

        # Cleanup for next seed
        train_env.close()
        #eval_env.close()

    print("\nAll seeds finished successfully.")
